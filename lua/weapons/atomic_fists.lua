AddCSLuaFile()
-- default 'fists' swep from sandbox, but it generates explosions the owner is immune to

SWEP.PrintName = "Atomic Fists" 
SWEP.Author = "Kilburn, robotboy655, MaxOfS2D & Tenrys, Nova Astral"
SWEP.Purpose = "Explode your enemies"
...
function SWEP:Initialize()
    ...
    if SERVER then
        hook.Add("EntityTakeDamage","atomic_fists_dmghook"..self:EntIndex(), function(hurtply,dmginfo)
            if(hurtply == self:GetOwner() and dmginfo:GetInflictor() == self) then
                return true
            end
        end)
    end
...
function SWEP:PrimaryAttack(right)
    ...
    if(tr.Hit and tr.HitPos:Distance(tr.StartPos) <= self.HitDistance+20) then
        timer.Simple(0.1,function() --small delay for the punch anim
            self:EmitSound("phx/explode06.wav",80,100,1)

            if SERVER then
                util.BlastDamage(self,self:GetOwner(),tr.HitPos,500,200)
                print("explod")
            end
...
function SWEP:OnDrop()
    if SERVER then
        hook.Remove("EntityTakeDamage","atomic_fists_dmghook"..self:EntIndex())
    end
